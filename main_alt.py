    ...
    if file_path:
        ...
        with open(file_path, 'w', newline='') as file:
            ...
            for i in range(len(button_text_keys)):
                ...
                writer.writerow({'name': name, 'weight': weight, 'on/off': checked, 'game': selected_tab_text, 'version': version})
...
#                 # select the appropriate tab based on the game value
#                 if game == "MP4":
#                     version_var.set(version)
#                     tab_parent.select(mp4_tab)
#                     clear_selections("MP4")
...
#                 for key in button_text_keys:
#                     print(f"Key: {key}")
#                     print(button_text_keys.index(name))
#                     if key == name:
#                         set_button_and_entry(game, button_text_keys.index(name), weight, checked)
...
def create_gui():
    ...
    save_button = tk.Button(button_frame, text="Save CSV", command=lambda: save_csv(tab_parent, version_var))
    save_button.pack(side=tk.LEFT)

    # Create 'Load CSV' button and add to button_frame
    load_button = tk.Button(button_frame, text="Load CSV", command=lambda: load_csv(tab_parent, version_var))
